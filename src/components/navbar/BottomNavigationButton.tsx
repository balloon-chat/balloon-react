...
export const BottomNavigationButton = ({ imgSrc, linkTo, label, current }: Props) => {
  return (<Link href={linkTo}>
    <LinkContainer isActive={current ?? false}>
      <SVGIcon url={imgSrc} color={current ? 'rgba(0,0,0,1)' : 'rgba(0,0,0,.6)'}/>
      <Label>{label}</Label>
    </LinkContainer>
  </Link>);
};
...
// tslint:disable-next-line:variable-name
export const MyPageNavigationButton = ({ linkTo, label, current }: MyPageButtonProps) => {
    ...
    <LinkContainer isActive={current ?? false}>
      {
        photoUrl
            ...
            : <SVGIcon url={svgImagePath.profile} color={current ? 'rgba(0,0,0,1)' : 'rgba(0,0,0,.6)'}/>
      }
      <Label>{label}</Label>
    </LinkContainer>
  </Link>);
